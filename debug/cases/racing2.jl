...
@info "---  GNEP SIM ---"
sim_results_gnep = solve_simulation(probs, 100; x0, only_want_gnep=true);

@info "---  BILEVEL SIM---  "
sim_results_bilevel = solve_simulation(probs, 100; x0, only_want_gnep=false);

animate(probs, sim_results_gnep; save=false, filename="results/racing3_gnep.mp4");
animate(probs, sim_results_bilevel; save=false, filename="results/racing3_bilevel.mp4");
