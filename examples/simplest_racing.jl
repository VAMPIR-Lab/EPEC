...
function dyn(T, X, U, x0, Δt; xdim=2, udim=2)
    x_prev = x0
...
function responsibility(T, Xa, Xb; xdim=2)
    mapreduce(vcat, 1:T) do t
...
function g1(z;
    Δt=0.1,
    r=1.0)
    #(; T, Xa, Ua, Xb, x0a) = view(z) # why doesn't this work??
    xdim=2;udim=2;
    T = Int((length(z) - 2 * xdim) / (2 * (xdim + udim)))
    @inbounds Xa = @view(z[1:xdim*T])
    @inbounds Ua = @view(z[xdim*T+1:(xdim+udim)*T])
...
function g2(z;
    Δt=0.1,
    r=1.0)
    #(; T, Xa, Xb, Ub, x0b) = view(z) # why doesn't this work??
    xdim=2;udim=2;
    T = Int((length(z) - 2 * xdim) / (2 * (xdim + udim)))
    @inbounds Xa = @view(z[1:xdim*T])
    @inbounds Ua = @view(z[xdim*T+1:(xdim+udim)*T])
...
function f2(z; α1=1.0, α2=0.0, β=1.0, xdim=2, udim=2)
    (; T, Xa, Xb, Ub) = view_z(z)
    running_cost = 0.0
...
function setup(; T=10,
    ...
    lb = [fill(0.0, xdim * T); fill(0.0, T); fill(-v_max, T); fill(-v_max, T); fill(-lat_max, T)]
    ub = [fill(0.0, xdim * T); fill(Inf, T); fill(+v_max, T); fill(+v_max, T); fill(+lat_max, T)]
    ...
    problems = (; gnep, bilevel, extract_gnep, extract_bilevel, OP1, OP2, params=(; T, Δt, r, lat_max))
end

function solve_seq(probs, x0; xdim=2, udim=2)
